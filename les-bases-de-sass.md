...
### À quoi ça sert ?
En fait, Sass ajoute aux CSS un ensemble de fonctionnalités qui permettent d'organiser de manière plus maintenable les feuilles de style. Sa devise  pourrait être "Don't Repeat Yourself" (ne vous répétez pas). Il ne s'agit donc pas de rajouter des propriétés, mais d'aider le développeur à y voir plus clair, à défricher son code (d'où la métaphore de la forêt vierge, CQFD).

Comme tout est plus simple avec un exemple, je vous propose comme fil rouge à ce cours le site Web de *Citron Inc*, une petite entreprise spécialisée dans la production de limonades biologiques. Plutôt que de rire bêtement, observez cette maquette :
...
Pour finir ce petit tour d'horizon, il faudrait aussi que je vous parle de Compass, un framework pour Sass. Compass ajoute un ensemble de fonctionnalités (des codes prêts à l'emploi) couvrant un large panel : cela va du reset CSS à la création de sprites d'images, en passant par tout un tas de trucs utiles (listes horizontales, footer "scotché" au bas de la page, typographie des liens, etc.).
...
## Prise en main
Dans ce chapitre, nous allons installer Sass et Compass. Nous verrons ensuite comment créer et compiler un projet.
...
+ Sous Linux, il est fort possible que votre distribution fournisse Ruby par défaut. Sinon, il est disponible dans les dépots officiel et s'installe via un gestionnaire de paquets (`apt`, `dnf`, etc.).
...
Maintenant, nous pouvons installer Sass et Compass avec `gem`. Gem est le gestionnaire de paquets de Ruby. Si c'est votre première utilisation de la ligne de commande, jeunes padawans, pas de panique : c'est d'une simplicité déconcertante. Lancez un terminal (sous Windows, vous pouvez utiliser `cmd` ou bien PowerShell) et entrez : `gem install compass` (attention à ne pas oublier `sudo` sur les sysèmes Unix).
...
Pourquoi installe-t-on Compass directement ? En fait, `gem` sait que Compass a besoin de Sass pour fonctionner, et installe donc Compass et Sass. Ce qui veut dire que l'installation est déjà terminée !

###Notre projet fil rouge
Il ne nous reste plus qu'à créer un projet qui nous servira de test durant le tutoriel. De nouveau, nous avons besoin de la console. Ouvrez un terminal dans votre dossier personnel (sous Windows, il est possible de choisir "Ouvrir une fenêtre de commandes ici" en effectuant un ||Maj|| + Clic-droit dans l'explorateur de fichier).
...
Vous remarquerez par ailleurs que le dossier "sass" contient trois fichiers SCSS : *screen.scss* (la feuille de style principale), *print.scss* (pour les styles spécifiques à l'impression) et *ie.scss* (pour les *hacks* spécifiques à Internet Explorer). C'est une suggestion d'organisation de votre projet, mais elle n'est absolument pas obligatoire. D'ailleurs, je vous invite à supprimer *screen.scss* et *ie.scss*, nous n'allons pas en avoir besoin pour la suite du cours.

Revenons à notre fil rouge : le site de limonade. Je vais vous fournir son code comme exemple tout au long de ce cours. Nous partirons donc d'un gros fichier CSS (et de la page HTML qui va avec) et nous allons chercher à le rendre le plus lisible, compréhensible et maintenable possible avec l'aide de Sass. Cela vous permettra de voir progressivement comment vous pouvez vous faciliter la vie avec Sass, ainsi que d'adopter quelques reflexes. Vous pouvez récupérer l'ensemble du projet sur [Github](https://github.com/Matouche/CitronInc).

Copiez le dossier `img` et la page HTML dans le dossier `tuto-sass` puis mettez le contenu de la feuille de style dans *screen.scss* (notez bien l'extension **.scss**).

Voilà, nous sommes prêts à commencer ! Si vous préférez démarrer directement avec l'un de vos projets actuels plutôt que de prendre mon exemple, n'hésitez pas : vous ne devriez pas avoir trop de mal à appliquer ce que je vais vous montrer.

### La compilation
Maintenant que le dossier de notre projet est prêt, nous pouvons compiler une première fois notre feuille de style, c'est-à-dire à générer  `stylesheets/screen.css`à partir de `sass/screen.scss`. Vu que nous avons créé notre projet avec la commande `compass`, nous pouvons aussi le compiler avec.

Tout d'abord, plaçez-vous  à l'intérieur du dossier `tuto-sass` en ligne de commande, avec la commande `cd tuto-sass`.

Entrez la commande `compass watch` (à retenir !). Celle-ci va indiquer à Compass qu'il doit *écouter* le projet. Désormais, s'il remarque une modification de vo, il lancera la compilation des feuilles de style concernées. Vous n'avez donc plus à vous soucier de la compilation, puisqu'elle est effectuée de manière transparente à chaque sauvegarde.
...
Je ne vous ai pas dit tout ce qu'il y avait à savoir sur les variables.
...
* Je n'ai pas intégré la concaténation, opération qui permet d'assembler deux chaînes de caractères, car son intérêt est plus que limité par rapport à l'interpolation.
* Enfin, je me suis retenu de vous parler du concept de portée des variables, parce que c'est là aussi peu utile à votre niveau.
...
## Les mixins (1/2)
Si une variable est bien utile pour stocker une valeur, que faut-il utiliser pour stocker un bloc de code entier afin de faciliter sa réutilisation ? Un indice : la réponse est dans le titre de ce chapitre.
